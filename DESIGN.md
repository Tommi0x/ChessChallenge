...
colors:
  jungle-field: "#102a1d"
  jungle-ink: "#e9f2e6"
  jungle-ink-dim: "#9db3a2"
  jungle-accent: "#d8a63c"
  jungle-accent-ink: "#0a1410"
  cave-field: "#241408"
  cave-ink: "#f2e6d6"
  cave-ink-dim: "#b39d84"
  cave-accent: "#d2542a"
...
- **Display Hero** (800, fluid to 5rem, 5.5rem at ≥1024px, 0.92 line-height): the start title only. Its second line is set in the accent as a block element.
...
- **Body** (400, 1rem/1.45): taglines, details, errors. The start blurb runs one step up and caps at 30rem.
...
**The Shared Measure Rule.** Anything sitting in the main column is `width: var(--board-size)`. Never hard-code a container width beside the board; a second measure breaks the pin.

**The One Opponent Rule.** Exactly one opponent is on screen at any moment. The full ladder appears once, on the start screen, as a wrapped list of names with 3px dot separators — never as a grid of cards, never as persistent chrome beside the board.
...
- **Focus:** the global ring — `2px solid var(--accent)` at 3px offset with a 0.25rem radius, applied to every link, button and `[tabindex]`.
...
- **Don't** show more than one opponent at a time, and don't reintroduce the roster grid or the static chess-app shell.
